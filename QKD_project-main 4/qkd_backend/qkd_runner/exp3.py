...
def run_exp3(message=None, bit_num=64, shots=1024, rng_seed=None):
    ...
    qc2_isa = pm.run(qc2)
    job2 = sampler.run([qc2_isa], shots=1)
    counts2 = job2.result()[0].data.c.get_counts()
    key2 = list(counts2.keys())[0]
    bmeas = list(key2)
    bbits = [int(x) for x in bmeas][::-1]

    # Sifting: keep only positions where Alice & Bob used same basis
    agoodbits = []
    bgoodbits = []
    match_count = 0
